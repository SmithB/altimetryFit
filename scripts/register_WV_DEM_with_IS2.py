...
def write_output(D_out, filename, D_pt, mask):
...
def register_one_DEM(DEM_file=None, GeoIndex_wc=None,
                     ...
                     sigma_min=0.1, sigma_max=5, verbose=False):
    D_out={'x':np.NaN,
           'y':np.NaN,
           'N':0,
           'N_days': 0,
    ...
    if D_pt.size < 5:
        ...
        write_output(D_out, DEM.filename, D_pt, np.ones(D_pt.size, dtype=bool))
        return
    ...
    if np.sum(mask0) < 5:
        D_pt.assign({'r':np.zeros_like(D_pt.x)+np.NaN})
        print('register_WV_DEM_with_IS2.py: not enough valid points found for ' + DEM.filename)
        write_output(D_out, DEM.filename, D_pt, mask0)
        return
    ...
    if np.sum(mask0) < 55:
        print('register_WV_DEM_with_IS2.py: not enough valid points found for ' + DEM.filename)
        write_output(D_out, DEM.filename, D_pt, np.ones(D_pt.shape, dtype=bool))
        return
    ...
    D_out={'x':np.mean(D_pt.x),
           ...
           'dh_dt':m[3],
           'delta_h':m[0]}
    D_BM=write_output(D_out, DEM.filename, D_pt, mask0)
